...
sys.path.insert(1, '/Users/tbartlett/Documents/Scripts')
...
def google_Info(email):
    ...
    # time.
    if os.path.exists('/Users/tbartlett/Documents/Scripts/token.json'):
        creds = Credentials.from_authorized_user_file('/Users/tbartlett/Documents/Scripts/token.json', SCOPES)
...
def jamf_Department(uapi_token, name):
    ...
    if response.status_code == 200:
        ...
        clean_name, title = xml_Characters(name)
        body = f"""
        <department>
...
def xml_Characters(department, title=""):
    if "&" in department:
